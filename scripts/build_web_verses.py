"""Build bible/web_verses.json from public-domain World English Bible (WEB).

Output format matches bible/gae_verses.json / bible/bsb_verses.json:
...
TMP_DIR = ROOT / "bible" / "_web_tmp"
CACHE_PATH = TMP_DIR / "web.json"
OUT_PATH = ROOT / "bible" / "web_verses.json"

# OSIS-style book codes from midvash WEB -> Korean short keys (gae order)
...
def convert(source: dict) -> dict[str, dict[str, dict[str, str]]]:
    ...
    for code, ko in BOOK_TO_KO:
        ...
        if book is None:
            raise SystemExit(f"Missing WEB book: {code}")
