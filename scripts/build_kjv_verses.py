"""Build bible/kjv_verses.json from public-domain King James Version (KJV).

Output format matches bible/gae_verses.json and other offline English JSON files:
...
TMP_DIR = ROOT / "bible" / "_kjv_tmp"
CACHE_PATH = TMP_DIR / "kjv.json"
OUT_PATH = ROOT / "bible" / "kjv_verses.json"

# OSIS-style book codes from midvash KJV -> Korean short keys (gae order)
...
def convert(source: dict) -> dict[str, dict[str, dict[str, str]]]:
    ...
    for code, ko in BOOK_TO_KO:
        ...
        if book is None:
            raise SystemExit(f"Missing KJV book: {code}")
